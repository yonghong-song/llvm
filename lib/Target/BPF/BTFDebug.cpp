...
void BTFTypeArray::getLocInfo(uint32_t Loc, uint32_t &LocOffset,
  ...
  LocOffset = (Loc * ATy->getSizeInBits() / ArrayInfo.Nelems) >> 3;
}
    ...
    if (CheckPointer) {
      ...
      if (Base) {
        if (const auto *CTy = dyn_cast<DICompositeType>(Base)) {
          ...
          if ((Tag == dwarf::DW_TAG_structure_type || Tag == dwarf::DW_TAG_union_type)
              && !CTy->isForwardDecl()) {
            auto FwdTypeEntry = llvm::make_unique<BTFTypeFwd>(CTy->getName(),
                                                           Tag == dwarf::DW_TAG_union_type);
            uint32_t FwdTypeId = addType(std::move(FwdTypeEntry));
            TypeEntry->setPointeeType(FwdTypeId);
        ...
        for (int I = 0; I < 2; I++) {
          for (const auto &StructType : StructTypes) {
            if (StructType->getName() == StructName) {
              ...
              OffsetReloc.OffsetNameOff = addString(OffsetReloc.TypeName.substr(0, Start) +
