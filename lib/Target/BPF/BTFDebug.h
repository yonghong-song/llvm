...
struct BTFOffsetReloc {
  ...
  std::string OrigTypeName; ///< Original type name
  std::string TypeName;     ///< Type name in the relocation record
  uint32_t TypeID;          ///< Type ID
  uint32_t OffsetNameOff;   ///< The string to traverse types
  uint32_t Dependency;      ///< Dependency
};
...
class BTFDebug : public DebugHandlerBase {
  ...
  void visitTypeEntry(const DIType *Ty, uint32_t &TypeId, bool CheckPointer, bool SeenPointer);
  ...
  void visitDerivedType(const DIDerivedType *DTy, uint32_t &TypeId, bool CheckPointer, bool SeenPointer);
  /// @}
